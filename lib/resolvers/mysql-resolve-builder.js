...
function ensureResultDateObject(jsonResult, node) {
...
function ensureListResultDateObject(resultList, node) {
  for (let i = 0, l = resultList.length; i < l; i++) {
    ensureResultDateObject(resultList[i], node);
  }
}
    ...
    return {
      ...
      treeChildren(parentNode, node) {
        ...
        return async function resolve(...resolveArgs) {
          ...
          ensureListResultDateObject(docs, node);
      ...
      item(parentNode, node) {
        ...
        return async function resolve(...resolveArgs) {
          try {
            ...
            ensureResultDateObject(jsonResult, node);
