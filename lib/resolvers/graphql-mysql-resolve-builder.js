...
const MysqlSchemaJournalAdapters = require('../db/mysql/mysql-schema-journal-adapters.js');
...
const buildFilterArgs = require('./helpers/build-filter-args.js');
...
module.exports = async function GraphqlMysqlResolveBuilder(database) {
  try {
      ...
      list(parentNode, node) {
        ...
        return async function resolve(parentQueryResult, args, context, _info) {
          try {
            ...
            log.debug('parentQueryResult', parentQueryResult);
            log.debug('args             ', args);
