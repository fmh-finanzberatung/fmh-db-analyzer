...
module.exports = function NodeEdgeInspector(inspectedNode, journal) {
  // create handler map { association: [], ... }
  //console.log('----------------', inspectedNode.name(), '-------------');
  ...
  const pub = {
    ...
    run() {
      ...
      Array.from(journal.entries())
        ...
        .filter((journalNode) => {
          //if (journalNode.name() === 'parent') return false;
          //console.log('journalNode.name()  ', journalNode.name());
          //console.log('inspectedNode.name()', inspectedNode.name());
          // ignore the journal node if it is identical with
          // the currently inspected node
          //fireEvent('hasMany', journalNode);
          //console.log('journalNode.edges(): ', journalNode.edges());
          //console.log('inspectedNode.name() ', inspectedNode.name());
        ...
        .forEach((neighbourNode) => {
          //console.log('inspectedNode.name()  ', inspectedNode.name());
          //console.log('neighbourNode.name()  ', neighbourNode.name());
          // must fire
          fireEvent('association', neighbourNode);
          ...
          neighbourNode
            ...
            .filter((yonderEdge) => {
              ...
              //console.log('yonderEdge          ', yonderEdge);
              //console.log('neighbourNode.name()', neighbourNode.name());
            ...
            .forEach((yonderEdge) => {
              ...
              // must fire
              // fireEvent('association', yonderNode);
