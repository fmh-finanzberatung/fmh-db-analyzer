import graphNodeName from './graph-node-name.js';
import capitalizeSnake from '../utils/capitalize-snake.js';

export default function GraphNode(tableName, tableDef, dbSupportFnc) {
  ...
  if (!dbSupportFnc) {
    throw new Error('missing dbSupportFnc (DatabaseSupportFunction)');
  }
  ...
  const proto = {
    ...
    treeEdges() {
      throw new Error('treeEdges must be implemented via dbSupportFnc');
    },
    name() {
