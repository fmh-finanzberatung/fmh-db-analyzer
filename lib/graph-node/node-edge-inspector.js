...
export default function NodeEdgeInspector(inspectedNode, journal) {
  const handlers = nodeRelations.reduce((obj, a) => {
    obj[a] = [];
    return obj;
  }, {});

  //log.info('inspectedNode', inspectedNode);
  ...
  const pub = {
    ...
    run() {
      //log.info('inspectedNode.edges()', inspectedNode.edges());
      inspectedNode.edges().filter((neighbourEdge) => {
        ...
        */
        ...
        if (!neighbourNode) {
          log.warn(
            `Expected a node for neighbourEdge ${neighbourEdge} but could not find it`
          );
          return false;
        }
      ...
      inspectedNode.treeEdges().filter((neighbourEdge) => {
        //log.info('neighbourEdge', neighbourEdge);
        fireEvent('parent', inspectedNode);
        fireEvent('children', inspectedNode);
        //fireEvent('root', inspectedNode);
        //fireEvent('roots', inspectedNode);
