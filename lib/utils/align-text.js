...
function padAlign(sNew, newOptions = {}) {
  ...
  const ensuredMaxWordEllipsis = ensureMaxWordLengthEllipsis(sNew, {
...
module.exports = {
  AlignText(s, newCommonOptions) {
    ...
    return {
      getSingleLine() {
        const ensuredEllipsisText = ensureMaxWordLengthEllipsis(s, {
      ...
      getMultiLine(newOptions = {}) {
        ...
        for (let i = 0, l = wrappedText.paragraphs.length; i < l; i++) {
          ...
          for (let j = 0, jl = p.length; j < jl; j++) {
            const line = p[j];
            const ensuredEllipsisText = ensureMaxWordLengthEllipsis(line, {
              maxWidth: availableWidth,
            });
