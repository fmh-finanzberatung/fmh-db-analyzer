...
import HashKey from '../utils/hash-key.js';
import { merge } from 'webpack-merge';
const env = process.env.NODE_ENV || 'development';
const config = await import(`../../config/env/${env}-config.js`);
const hashSalt = config.hashSalt;
const knexfile = await import(`../../knexfile-${env}.js`);
import Knex from 'knex';
import CalC from '../../lib/utils/cal-c.js';
import destructureDate from '../../lib/utils/destructure-date.js';
import DbHelpers from '../db/mysql/db-helpers.js';
const knex = Knex(knexfile.default);
const { insert } = DbHelpers(knex);
import RegistrationMailer from '../mailers/registration.mailer.js';
...
export default function DbAuthPlugin(optionArgs = {}) {
