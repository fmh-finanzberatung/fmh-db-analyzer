...
const mongoTypesMap = DbToGraphqlTypesMap('mongo');
    ...
    try {
      ...
      journal.forEach((node) => {
        ...
        node.domesticAttributes().forEach((attr, fieldName) => {
          ...
          const graphqlInputType = mongoTypesMap.gqlInputType(dbDataType);

          graphqlTypeBuilder.addInputFieldDef(fieldName, graphqlInputType);
          const graphqlParamType = mongoTypesMap.gqlType(dbDataType);
